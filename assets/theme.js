...
var MarqueeText = class extends HTMLElement {
  constructor() {
    ...
    __privateAdd(this, _currentAnimation, void 0);
...
// --- Load More Products functionality for collection page ---
(function () {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    let currentPage = 2;
    const productsPerPage = remainingCountSpan
      ? parseInt(remainingCountSpan.getAttribute("data-loaded"))
      : 12;
    let loading = false;
    let total = remainingCountSpan
      ? parseInt(remainingCountSpan.getAttribute("data-total"))
      : 0;
    let loaded = remainingCountSpan
      ? parseInt(remainingCountSpan.getAttribute("data-loaded"))
      : 0;
    ...
    function updateRemaining() {
      ...
      if (remainingCountSpan) {
        ...
        if (remaining <= 0) {
          ...
          remainingCountSpan.parentElement.style.display = "none";
        }
      }
    }
    ...
    if (loadMoreBtn && productList) {
      loadMoreBtn.addEventListener("click", async function () {
        ...
        try {
          ...
          // Find the product-list in the response
          const newProductList = tempDiv.querySelector("product-list");
          if (newProductList) {
            const newProducts =
              newProductList.querySelectorAll(".product-card");
            if (newProducts.length > 0) {
              newProducts.forEach((card) => {
                productList.appendChild(card.cloneNode(true));
              });
...
var BeforeAfter = class extends HTMLElement {
  constructor() {
    ...
    __privateAdd(this, _onKeyboardNavigation2);
    ...
    this.addEventListener(
      "keydown",
      __privateMethod(this, _onKeyboardNavigation2, onKeyboardNavigation_fn2)
    );
  }
  connectedCallback() {
    inView9(
...
_onKeyboardNavigation2 = new WeakSet();
onKeyboardNavigation_fn2 = function (event) {
  ...
  if (this.hasAttribute("vertical")) {
    ...
    newPosition =
      event.code === "ArrowLeft" ? currentPosition - 1 : currentPosition + 1;
  }
  this.style.setProperty(
